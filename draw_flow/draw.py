...
def arrow_from_to(destination, from_box, to_box, color, factor):
    ...
    if destination == "side":
        ...
        draw.polygon(
            [
                (x_to_1, y_end),
                (x_to_1 - arrow_size, y_end - arrow_size / 2),
                (x_to_1 - arrow_size, y_end + arrow_size / 2),
            ],
            fill=color,
        )
        ...
        draw.polygon(
            [
                (x_start2 - arrow_size / 2 / factor, y_to_1 - arrow_size / factor),
                (x_start2 + arrow_size / 2 / factor, y_to_1 - arrow_size / factor),
                (x_start2, y_to_1),
            ],
            fill=color,
...
def draw_workflow(offsets, workflow, y_start):
    ...
    try:
        ...
        run_box = draw_my_text_rectangle_width(
        ...
        for extra in extras:
            extra_box = draw_my_text_rectangle_width(
                offsets["python_step_offset"] + repeat_indent,
                y_start + extra_y,
                offsets["max_step_width"],
                extra,
        ...
        draw.polygon(
            [
                (x_left, y_repeat),
                (x_left - arrow_size / 2 / factor, y_repeat + arrow_size / factor),
                (x_left + arrow_size / 2 / factor, y_repeat + arrow_size / factor),
            ],
            fill=blue,
        )
    ...
    name_box = draw_my_text_rectangle_width(
        offsets["workflow_offset"], y_start + y_size, max_size, name, green
    )
    ...
    for action in actions:
        ...
        text_box = draw_my_text_rectangle_width(
            offsets["action_offset"],
            y_start + y_size,
            offsets["max_action_width"],
            action_name,
            red,
        )
        draw_text(
            offsets["component_offset"], y_start + y_size + text_y_offset, instrument
        )
        ...
        try:
            ...
            draw_text(
                offsets["component_offset"] + arial.getsize(instrument)[0],
                y_start + y_size + text_y_offset,
                text,
            )
        except:
            pass
        ...
        try:
            ...
            protocol_box = draw_my_text_rectangle_width(
                offsets["action_offset"] + 20,
                y_start + y_size,
                protocol_size + 2 * text_x_offset,
                protocol,
                orange,
            )
...
def get_sizes(plot):
    ...
    max_workflow_name_width = max(
        [arial.getsize(x["name"].replace(".yaml", ""))[0] for x in plot["workflows"]]
    )
    ...
    action_offset = (
        ...
        + python_width
        + inter_column_width
        + max(max_workflow_name_width, workflow_header_width)
        + inter_column_width
    )
...
def process_file(spec_file, output_file, no_transfer_flag):
    ...
    python_box = draw_my_text_rectangle(
        python_offset, python_y_location, python_program, blue
    )
...
def main(argv):
    parser = argparse.ArgumentParser(
        description="Program to generate plots for WEI workflows"
    )
