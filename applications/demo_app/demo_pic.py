...
def main():
    wf_path = Path(
        "/home/rpl/workspace/rpl_workcell/pcr_workcell/workflows/demo_pic.yaml"
    )
    ...
    payload = {}
    run_info = wei_client.run_workflow(payload=payload)
    iter_thread = ThreadWithReturnValue(
        target=wei_run_flow, kwargs={"wf_file_path": wf_path, "payload": payload}
    )
    iter_thread.run()
    ...
    with open(
        "/home/rpl/experiments/PCRTest"
        / Path(run_info["run_dir"].name)
        / "wf_steps.txt",
        "w",
    ) as f:
        ...
        f.write(report_js)
    shutil.copy2(
        run_info["run_dir"] / "results" / "final_image.jpg",
        "/home/rpl/experiments/PCRTest"
        / Path(run_info["run_dir"].name)
        / "final_image.jpg",
    )
    publish_iter(
        "/home/rpl/experiments/PCRTest" / Path(run_info["run_dir"].name),
        "/home/rpl/experiments/PCRTest",
    )
