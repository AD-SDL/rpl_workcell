...
def iter_run(init_file, iteration):
    ...
    preds_df = pd.concat(
        [
            ...
            pd.DataFrame(preds_inv_scaled, columns=["L", "a", "b"]),
        ],
        axis=1,
    )
    ...
    top_candidates = select_next_exp_ml(
        preds_df, target_Lab, uncertainties, iteration
    ...
    df_pivot.to_csv(
        f"/home/rpl/workspace/polybot_workcell/polybot_app/demo_files/for_chemspeed/loop_{iteration+1}.csv",
        index=False,
    )

    # # 5. Call the globus data transfer to send the file from the local folder to Chemspeed
    local_path = Path(
        "/home/rpl/workspace/polybot_workcell/polybot_app/demo_files/for_chemspeed"
    )
    ...
    if 1 in peak_check_result or -1 in peak_check_result:
        ...
        df = tecan_read(
            filename
        )  # updates the file with the Tecan measurements to be used further
    ...
    peak_check_result = peak_check(df)
    quality_check_idx = [
        index for index, value in enumerate(peak_check_result) if value in [1, -1]
    ]
    top_candidates_updated = (
        top_candidates.reset_index().drop(quality_check_idx).reset_index()
    )
    print("top_candidates_updated", top_candidates_updated)
    lab_values_updated = [
        lab_values[i] for i in range(len(lab_values)) if i not in quality_check_idx
    ]
    ...
    X_train, y_train = (
        get_train_data_representation_dft(top_candidates.reset_index()),
        lab_values,
    )
    train_data_1, train_data_2, train_data_3, y_train = (
        np.array(X_train.iloc[:, :Ndims].values, dtype=np.float16),
        np.array(X_train.iloc[:, Ndims : 2 * Ndims].values, dtype=np.float16),
        np.array(X_train.iloc[:, 2 * Ndims :].values, dtype=np.float16),
    ...
    torch.save(
        ...
        os.path.join(
            "/home/rpl/workspace/polybot_workcell/polybot_app/ml_loop/set_transformer_checkpoints",
            "set_transformer_dft_ecfp_3.tar",
        ),
    )

    # At the end of each loop return the updated init.json file
    with open(
        "/home/rpl/workspace/polybot_workcell/polybot_app/demo_files/init.json",
        "w",
        encoding="utf-8",
    ) as f:
...
def polybot_wei_workflow(
    file_name=None,
    tecan_file_name=None,
    loop_file_path=None,
    tecan_file_path=None,
    tecan_iteration=None,
):  # we can add the iteration here and read the yaml based on the iteration we are
    ...
    if not tecan_file_name:
        tecan_file_name = (
            "C:/Users/Public/Documents/Tecan/Magellan Pro/mth/ECP-350-800-Yukun.mth"
        )
...
# all_possibilities['exclude'] = 0
# all_possibilities = all_possibilities.reset_index(drop=True)
all_possibilities = pd.read_csv(
    "updated_csv1.csv"
)  # ('/home/rpl/workspace/all_possibilities_2_loop.csv')
